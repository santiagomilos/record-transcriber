...
**Distribution:** a prebuilt arm64 bundle, zipped with `ditto` and attached to a public GitHub Release under one fixed asset name, `Record-Transcriber-arm64.zip`, so `releases/latest/download/` always resolves. `make release VERSION=x.y.z` builds it on the maintainer's machine: there is no CI. The version is not in the source `Info.plist` (placeholders) but stamped from the tag at build time, with the commit count as `CFBundleVersion`, before signing, because the plist is sealed into the signature. The app stays ad-hoc signed and is not notarized, so `install.sh`, the one-command installer, strips the quarantine flag after downloading; a browser download of the same zip has to be allowed under Privacy & Security. The installer also installs Homebrew, `ffmpeg` and `whisper-cpp`, and checks Claude Code without installing it or signing in.
